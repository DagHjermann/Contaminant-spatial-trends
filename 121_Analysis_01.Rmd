...
dat2 <- dat %>%
  ...
  ungroup() %>%
  filter(!is.na(Dist_along_coast) & PARAM != "Sum PCB 7")
...
if (FALSE){
  ...
  # 'Sum PCB 7' and CB_S7
  # CB_S7 is duplicated as 'Sum PCB 7' in two years 2015 and 2016, so it can be ignored
  dat2 %>%
    filter(PARAM %in% c("Sum PCB 7", "CB_S7")) %>%
    xtabs(~PARAM + MYEAR, .)

  dat2 %>%
    filter(PARAM %in% c("Sum PCB 7", "CB_S7") & MYEAR == 2015) %>%
