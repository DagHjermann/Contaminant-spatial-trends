...
sel1 <- unique(tab_comb$PARAM) %in% pars_pcb
sel2 <- unique(tab_comb$PARAM) %in% pars_bde
unique(tab_comb$PARAM)[!(sel1 | sel2)] 

#   kableExtra::group_rows(Substance.Group)


kabletab <- kable(tab_comb %>% select(-Substance.Group)) %>%
  kable_styling(bootstrap_options = "condensed")

# tab

for (category in levels(tab_comb$Substance.Group)){
...
pars_pcb <- tab_comb %>% 
  ...
  pull(PARAM) %>% 
  sort_by_number() %>%
  move_to_back("Pentaklorbenzen (QCB)")

pars_bde <- unique(tab_comb$PARAM) %>% grep("^BD", ., value = TRUE) %>%
  sort_by_number()

pars_hbcd <- unique(tab_comb$PARAM) %>% grep("HBCD", ., value = TRUE) %>%
  sort() %>%
...
sel1 <- unique(tab_comb$PARAM) %in% pars_pcb
sel2 <- unique(tab_comb$PARAM) %in% pars_bde
sel3 <- unique(tab_comb$PARAM) %in% pars_hbcd
tab_comb[!(sel1 | sel2 | sel3),] %>%
  select(Substance.Group, PARAM)
